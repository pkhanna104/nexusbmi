function [rch_abs_time, targ_inds, tm_bound, targ_start_abs_time] = calc_movement_time(blocks, day, bounds)
...
[ft, raw_td_m1, raw_td_stn, raw_pxx, abs_t, targ, curs, rew_inds, state,ix_bound] = parse_dat(blocks, day, bounds);
...
for j = 1:length(ix_bound)
    ix = ix_bound(j);
    for iii = 1:4
        subplot(2,2,iii);hold on;
        plot([abs_t(ix)/60, abs_t(ix)/60], [0, 1.2*max_tm(iii)],'--',...
